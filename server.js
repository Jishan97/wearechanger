...
var {
  defaultMsg
} = require('./model/defaultmsg');
var {
  defaultMeetingMsg
} = require('./model/defaultMeetingMsg');
var {
  defaultA
} = require('./model/defaultA');
var {
  defaultD
} = require('./model/defaultD');
var {
  adminPassword
} = require('./model/adminpassword');
var {
  moderatorPassword
} = require('./model/moderatorpassword');
...
app.post('/register', async (req, res) => {
  ...
  const fetchD = await User.find({
    email
  })
  const fetchD1 = await User.find({
    username
  })
